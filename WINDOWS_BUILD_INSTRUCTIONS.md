...
### Проблема 3: "Failed to execute script"

**Симптом:** При запуске .exe появляется ошибка

**Решение:**
1. Проверить что .env.example находится рядом с .exe
2. Пересобрать с флагом debug:
   ```cmd
   pyinstaller NOC-Toolkit.spec --clean --debug=all
   ```
3. Проверить логи в `dist\`
